...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    stat_tup_list = list(map(lambda stat: (stat[info['playerid']], formula(info, stat)),
                             statistics))
    
    stat_tup_list.sort(key=lambda tup: tup[1], reverse=True)
    
    return stat_tup_list[0:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    mastercsv = read_csv_as_list_dict(info["masterfile"], info['separator'], info['quote'])
    final_list = list()

    # Loop through top_ids_and_stats, search in master file, append to list
    for tup in top_ids_and_stats:
      # Find player id in mastercv
        for player in mastercsv:
            if player[info['playerid']] == tup[0]:
                stat = tup[1]
                fname = player[info['firstname']]
                lname = player[info['lastname']]
                final_list.append('{:.3f} --- {} {}'.format(stat, fname, lname))
          # Once the player is found stop searching and go for the next one
                break
    return final_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    stats = read_csv_as_list_dict(info["battingfile"], info['separator'], info['quote'])
    
    stats_by_year = filter_by_year(stats, year, info['yearid'])
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    #Loop through read csv file
    for player in statistics:
        player_id = player[playerid]
      # Check if player_id is already added in the final dictionary
      # Aggregate field (sum)
        if(player_id in final_dict):
            for field in fields:
                final_dict[player_id][field] += int(player[field])
      # Otherwise add them to it for the first time 
        else:
            final_dict[player_id] = {}
            for field in fields:
                final_dict[player_id][playerid] = player_id
                final_dict[player_id][field] = int(player[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    player_id_aggregate = aggregate_by_player_id(stats, info['playerid'], info["battingfields"]) 
    
    top_ids_and_stats = top_player_ids(info, player_id_aggregate.values(), formula, numplayers)
